...
import random
import einops
from llava.model.compressor import VideoTokenCompressor

# Union-Find Data Structure with path compression and batch merge by rank optimization
class UnionFind:
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            # print(getattr(config, "mm_patch_merge_type", ""))
            # exit()
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            image_features = []
            mm_use_zip = getattr(self.config, "mm_use_zip", False)
            ...
            if mm_patch_merge_type == "flat":
                ...
                ori_token_num = image_features[0].shape[1]
                image_features = [x.flatten(0, 1) for x in image_features]
                
                if mm_use_zip:
                    # Get similarity threshold tau and error tolerance eplsion
                    tau = getattr(self.config, "mm_zip_tau", 0.95)
                    epsilon = getattr(self.config, "mm_zip_epsilon", 0.05)
                    # print(f"Using tau={tau}, epsilon={epsilon}")
                    # exit()

                    new_image_features = []
                    for idx, image_feat in enumerate(image_features):
                        rank0_print("flat input size: ",image_feat.shape)
                        frame_num = image_feat.shape[0]//ori_token_num
                        total_samples = image_feat.shape[0]                   


                        
                        image_feat_hw = einops.rearrange(image_feat, '(n hw) c -> n hw c', n=frame_num, hw=ori_token_num)
                        all_fused_feature = []
                        sample_indices = []
                        frame_cnt = 0
                        
                        # Get binary similarity matrix A for all frames
                        ...
                        # Spatial Compression on each frame
                        for image_feat_per_frame in image_feat_hw:
                            high_similarity_indices_per_frame = high_similarity_indices[frame_cnt]
                            connected_components = approximate_components(high_similarity_indices_per_frame.cpu().numpy(), epsilon=epsilon)
                            # Merge connected components
                            fused_features = []
                            for connected_component in connected_components:
                                selected_features = image_feat_per_frame[connected_component]           
                            ...
                            frame_cnt += 1         
                        all_fused_feature = torch.cat(all_fused_feature)
                        selected_tokens = all_fused_feature
                        num_tokens = len(selected_tokens)
                        self.total_token_1 += len(selected_tokens)
                        self.val_cnt += 1
                        # print("feature after first zip: ",selected_tokens.shape)
                        # print("average token first step: ", self.total_token_1/self.val_cnt)

                        # Temporal compression
                        second_zip = True
                        if second_zip:
                            # Get binary similarity matrix A
                            ...
                            connected_component_select_tokens = approximate_components(high_similarity_indices_select_tokens.cpu().numpy(), epsilon=epsilon)
                            ...
                            num_tokens = len(selected_tokens)
                            self.total_token_2 += len(selected_tokens)
                            
                            print("average token first step: ", self.total_token_1/self.val_cnt, "average token second step: ", self.total_token_2/self.val_cnt)

                        # Token Merge for all tokens
                        ...
                        # Find closest indice in selected tokens for every token in remaining_values 
                        closest_indices = torch.argmax(similarity_matrix, dim=1)
                        
                        # Merge token with closest_indices
                        ...
                        image_feat = selected_tokens

                        new_image_features.append(image_feat)
                            
                    image_features = new_image_features
                rank0_print("flat after zip",image_features[0].shape)
                if mm_use_zip:
                    rank0_print("average token first step: ", self.total_token_1/self.val_cnt, "average token second step: ", self.total_token_2/self.val_cnt)
                ...
                for image_idx, image_feature in enumerate(image_features):
                    ...
                    if image_idx in video_idx_in_batch:  # video operations
                        ...
                        new_image_features.append(image_feature)
                image_features = new_image_features
                rank0_print("token size after spatial_unpad", image_features[0].shape)
            else:
                raise ValueError(f"Unexpected mm_patch_merge_type: {self.config.mm_patch_merge_type}")
        else:
            image_features = self.encode_images(images)
        # rank0_print("token size after merge", image_features[0].shape)
