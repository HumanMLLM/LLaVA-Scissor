            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if "zip" in model_name.lower():
                        from llava.model.language_model.llava_qwen_zip import LlavaQwenZipConfig
                        if overwrite_config is not None:
                            llava_cfg = LlavaQwenZipConfig.from_pretrained(model_path)
                            rank0_print(f"Overwriting config with {overwrite_config}")
                            for k, v in overwrite_config.items():
                                setattr(llava_cfg, k, v)
                            print("here")
                            model = LlavaQwenZipForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                        else:
                            model = LlavaQwenZipForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
