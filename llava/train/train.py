...
from llava.oryx_mm_utils import  process_anyres_highres_image_genli, process_anyres_video_genli, process_anyres_video_genli_long
...
class ModelArguments:
    ...
    mm_use_zip: Optional[str] = field(default=False)
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    im_start = tokenizer.additional_special_tokens_ids[0]
    im_end = tokenizer.additional_special_tokens_ids[1]
...
class LazySupervisedDatasetOryx(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments):
        super(LazySupervisedDatasetOryx, self).__init__()
    ...
    def process_image(self, image_file):
        if type(image_file) is str:
            ...
            image = read_image_patch(image_file, self.data_args.image_folder)
        else:
    ...
    def process_video(self, video_file):
        video = read_video_patch(video_file, self.data_args.video_folder)
        video_processed = []
    ...
    def process_video_pretrain(self, video_file, target_idx):
        ...
        return (video_processed, 147456, "video_long"), target_idx_new
...
class LazySupervisedDatasetMyOryx(Dataset):
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, data_args: DataArguments):
        super(LazySupervisedDatasetMyOryx, self).__init__()
        self.tokenizer = tokenizer
        self.list_data_dict = []
        ...
        if "image" in sources[0]:
            ...
            try:
                if "shareVideoGPTV" in video_file:
                    ...
                    video_data = read_video_patch(video_file, self.data_args.video_folder)
                    num_frames_of_video = len(video_data)
                    f_start = 0                       
                    f_end   = num_frames_of_video - 1 
                    frame_indices = list(range(f_start, f_end + 1))
                    duration = len(frame_indices)
                    video_temp = []
                    num_frames = 16
                    if num_frames is None:
                        sampled_frame_indices = [frame_indices[i] for i in frame_sample(duration, mode='fps', fps=1)]
                    else:
                    ...
                    for indice in sampled_frame_indices:
                        video_temp.append(video_data[indice])
                    video_data = video_temp
                    # print(len(video_data))

                processor = self.data_args.image_processor
                image = processor.preprocess(video_data, return_tensors="pt")["pixel_values"]
                if self.data_args.add_time_instruction:
                    time_instruciton = f"The video lasts for {video_time:.2f} seconds, and {num_frames_to_sample} frames are uniformly sampled from it. These frames are located at {frame_time}.Please answer the following questions related to this video."
                    sources[0]["conversations"][0]["value"] = f'{DEFAULT_IMAGE_TOKEN}\n{time_instruciton}\n{sources[0]["conversations"][0]["value"].replace(DEFAULT_IMAGE_TOKEN, "")}'
                image = [(image, video_data[0].size, "video")]
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
                # print(sources)
            except Exception as e:
                print(f"Error: {e}")
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    if "oryx" in data_args.data_path.lower():
        train_dataset = LazySupervisedDatasetMyOryx(tokenizer=tokenizer, data_path=data_args.data_path, data_args=data_args)
    else:
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if any(
        [
            ...
            model_args.mm_use_zip is not None,
        ]
    ):
        cfg_pretrained = AutoConfig.from_pretrained(model_args.model_name_or_path)

    if model_args.mm_use_zip is not None:
        overwrite_config["mm_use_zip"] = model_args.mm_use_zip
